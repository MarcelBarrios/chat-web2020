// Initialize Firebase
var config = {
  apiKey: "AIzaSyDqrrShLmNI-8lwLB3dSLEf5gcwOn70xKc",
  authDomain: "chat-514ab.firebaseapp.com",
  databaseURL: "https://chat-514ab.firebaseio.com",
  projectId: "chat-514ab",
  storageBucket: "chat-514ab.appspot.com",
  messagingSenderId: "77220997077"
};
...
var login = () => {
  firebase.auth().signInWithPopup(provider).then(function(result) {
    ...
    $("#login-screen").fadeOut(() => {
      $("#chat-screen").html(`<div id='#chat-screen-content'
          style='width: 100vw;
          height: 100vh;
          display: flex;
        ...
        $('#input-msg').keypress(function(e) {
          if (e.keyCode === 13)
            sendMessage(user.displayName, new Date(), user.email, user.photoURL);
          }
        );
...
var showMessages = (user) => {
  messages.on('value', function(snapshot) {
    ...
    for (var msgId in values) {
      ...
      var side = user.email === msg.email
        ? 'right'
        : 'left';
      var margin = user.email === msg.email
        ? 'margin-left: 15px;'
        : 'margin-right: 15px;';
      var corner = user.email === msg.email
        ? 'right-top'
        : 'left-top'

      $("#messages").append(`<div class="msg-div ${side}">
            <div style="${margin}">
